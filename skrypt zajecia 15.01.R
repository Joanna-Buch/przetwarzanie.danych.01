...
# wczytywanie danych
data_1 <- readxl::read_excel("R/data_msu.xlsx", sheet = "loi")

data_2 <- readxl::read_excel("R/data_msu.xlsx", sheet = "elemental")

data_3 <- readxl::read_excel("R/data_msu.xlsx", sheet = "bsi")

# Wylistowanie dostępnych arkuszy w pliku excel (podpowiadajka, żeby pokazało jakie są arkusze w pliku):
readxl::excel_sheets("R/data_msu.xlsx")

# Łączenie ramek (mutating join)
# Left join (łączenie danych z lewej strony, x i y)
...
# Łączenie do prawej right join
data_right_1 <- dplyr::right_join(data_1, data_2)

data_right_2 <- dplyr::right_join(x = data_3, y = data_1, by = dplyr::join_by(sample.id)) dplyr::right_join(data_2, by = dplyr::join_by(sample.id))
